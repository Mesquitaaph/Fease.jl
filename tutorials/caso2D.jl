...
function caso_2D()
    ...
    contorno_completo = reduce(vcat, getQuadSideNodes.(Nx1, Nx2, 1:4))
    nos_prescritos = [unique(contorno_completo)]
    fronteira = monta_fronteira_2D_uniforme(a, b, Nx1, Nx2, nos_prescritos)
    malha = monta_malha_2D_uniforme(baseType, Nx1, Nx2, 1, fronteira)
